...
import { createRemoteRunner } from "../src/remote-runner-factory";
...
const remoteExamples = LANGUAGE_EXAMPLES.filter(({ language }) => {
  ...
  return definition?.remoteAdapter !== undefined
    && language !== "dart"
...
for (const example of remoteExamples) {
  ...
  const runner = createRemoteRunner(language, nativeFetch);
  if (runner === null) throw new Error(`Missing remote adapter for ${language.id}`);
